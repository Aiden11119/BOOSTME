...
import { Check, X, Search, Clock, Calendar, Mail, User, ShieldAlert, BookOpen, GraduationCap } from 'lucide-react';
...
function VerificationRequests() {
  ...
  const handleApprove = async (id, name) => {
    if (!window.confirm(`Are you sure you want to APPROVE ${name}'s request? An account will be created with default password '123456'.`)) return;
    
    setActionLoadingId(id);
    try {
      const res = await api.post(`/admin/registration-requests/${id}/approve`);
      toast.success(res.data.message || 'Request approved and account created!');
      fetchRequests();
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-gray-500">
        ...
        <p className="font-semibold text-sm">Loading verification requests...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Manually review and approve pending registration requests from users who did not receive email codes.
          </p>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-100 text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 text-xs font-bold text-gray-400 uppercase tracking-wider">
              ...
              <th className="px-6 py-4">Submitted Reason/Feedback</th>
