...
const ManualVerificationModal = ({ initialEmail = '', onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const res = await api.post('/auth/submit-registration-request', formData);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fade-in-up">
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 max-w-lg w-full shadow-2xl border border-gray-100 dark:border-slate-700 relative overflow-hidden transition-all duration-300">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-xl font-extrabold text-gray-900 dark:text-white">Manual Verification Request</h3>
            <p className="text-xs text-gray-400 dark:text-slate-400 mt-0.5">Submit details for admin validation & approval</p>
          </div>
        </div>
